...
def generate_docs(source_path, include_patterns, exclude_patterns, language, max_size):
    ...
    try:
        ...
        for i, file_path in enumerate(
            files_to_process[:10]
        ):  # Limit to first 10 files to avoid long processing
            status_text.text(
                f"Processing file {i + 1}/{min(total_files, 10)}: {os.path.basename(file_path)}"
            )
            ...
            progress_bar.progress(0.3 + (0.6 * (i + 1) / min(total_files, 10)))
...
- Files processed: {min(total_files, 10)}
...
"""
        ...
        return (
            True,
            final_documentation,
            {"files_processed": min(total_files, 10), "total_files": total_files},
        )
    ...
    with col2:
        ...
        if source_path and st.button("🚀 Generate Documentation"):
            success, content, result = generate_docs(
                source_path, include_patterns, exclude_patterns, language, max_size
            )
            ...
            if success:
                ...
                if result:
                    st.info(
                        f"Processed {result['files_processed']} out of {result['total_files']} files"
                    )
